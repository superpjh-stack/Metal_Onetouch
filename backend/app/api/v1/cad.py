...
async def get_drawing(drawing_id: uuid.UUID, db: DBSession, _: CurrentUser):
    """도면 상세"""
    return await CadAnalysisService(db).get_drawing(drawing_id)


@router.get("/{drawing_id}/status", response_model=CadStatusResponse)
async def get_drawing_status(drawing_id: uuid.UUID, db: DBSession, _: CurrentUser):
    """분석 상태 polling 용"""
...
async def get_annotation_task(drawing_id: uuid.UUID, db: DBSession, _: CurrentUser):
    """도면의 어노테이션 태스크 조회"""
